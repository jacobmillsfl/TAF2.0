...
class MusicPlayer extends Component
{
  ...
  {
    return [
      "https://tafers.net/files/AbyssOfBliss.mp3",
  ...
  songClick(index)
  {
    this.setState((state) =>
    {
      console.log("Playing song: ", state.songs[index])
      return { url: state.songs[index] };
    });
  }
          ...
          <tbody style={musicPlayerBody}>
            {
              song_list.map((song, index) => (
                <tr key={index} onClick={() => { this.songClick(index); }}>
                  <td>TAF</td>
                  <td>{song}</td>
                </tr>
              ))
